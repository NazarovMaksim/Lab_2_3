...
function CreateOutpMassForDate(Date, int_mode_y, int_mode_x) { // a, b
    ...
    if(int_mode_y == 0) str_mode = "Предпочитаемый микрофон";
    else if (int_mode_y == 1) str_mode = "Год рождения";

    if(int_mode_x == 0) str_mode_date = "Всего часов записи";
    else if (int_mode_x == 1 ) str_mode_date = "Всего потрачено";
...
function DrawLinearGrafic_02(data_x, data_y, strX, strY, int_color) {
    ...
    svg.append("g")
        .attr("transform", "translate(" + 20 + ", "+ 210 + ") rotate(-90)")
        .append("text")
        .text(strX)
        .attr("class", "y-legend")
        .style("fill", "gray"); // set color to gray
...
function MainGenerateGrafic(input_a, input_b) {
    ...
    if(input_b == 0) strLett = "Год рождения";
    else if(input_b == 1) strLett = "Всего потрачено";
    
    if(input_a == 1) { 
        // Если выбран "Год выхода", то рисую линейный график
        DrawLinearGrafic_02(data_x, data_y, strLett, "Всего часов записи", input_b);
    } else { 
