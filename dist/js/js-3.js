...
function createTable(data) {
  ...
  var headerNames = ["ФИО Клиента", "Всего потрачено", "Всего часов записи", "Предпочитаемый микрофон", "Год рождения"];
...
function setSortTable(date, strMass) {
  ...
  for(let i = 0; i < date.length; i++) {
    ...
    if(strMass[0] != "") {
      ...
      if (date[i]["ФИО Клиента"].indexOf(strMass[0]) === -1) {
        bool = false;
      }
    }
    if(strMass[1] != "") {
      //console.log('strMass[1] = ' + strMass[1] );
      if (date[i]["Предпочитаемый микрофон"].indexOf(strMass[1]) === -1) {
        bool = false;
      }
    }
    if(strMass[2] != 0) {
      if(date[i]["Год рождения"] != strMass[2]) {
        bool = false;
      }
    }
    if(strMass[3] != 0) {
      if(date[i]['Всего потрачено'] < strMass[3]) {
        bool = false;
      }
    }
    if(strMass[4] != 0) {
      if(date[i]['Всего потрачено'] > strMass[4]) {
        bool = false;
      }
    }
    if(strMass[5] != 0) {
      if(date[i]['Всего часов записи'] < strMass[5]) {
        bool = false;
      }
    }
    if(strMass[6] != 0) {
      if(date[i]['Всего часов записи'] > strMass[6]) {
        bool = false;
      }
    }
...
allElemMass[0] = '--- Нет ---';
allElemMass[1] = 'ФИО Клиента';
allElemMass[2] = 'Всего потрачено';
allElemMass[3] = 'Всего часов записи';
allElemMass[4] = 'Предпочитаемый микрофон';
allElemMass[5] = 'Год рождения';
